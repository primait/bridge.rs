...
impl JsonWebKeySet {
    pub fn is_signed(&self, kid: &str) -> bool {
    ...
    pub async fn fetch(client: &Client, config: &Config) -> PrimaBridgeResult<Self> {
        client
            ...
            .send()
            .await
            .map_err(|e| PrimaBridgeError::Auth0JwksFetchError(config.jwks_url().clone(), e))?
            .json::<JsonWebKeySet>()
            .await
            .map_err(|e| {
                PrimaBridgeError::Auth0JwksFetchInvalidJsonError(config.jwks_url().clone(), e)
            })
    }
}
...
impl JsonWebKey {
    ...
    pub fn as_rsa_public_key(&self) -> PrimaBridgeResult<&RsaPublicJwk> {
        match self {
