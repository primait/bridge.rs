...
impl RedisCache {
    pub fn new_with_client(client: redis::Client, config: &Auth0Config) -> Self {
        Self {
            client,
            encryption_key: config.token_encryption_key().to_string(),
        }
...
//
//     use crate::auth0_config::Auth0Config;
//     use crate::cache::{Cacher, CacheEntry};
//     use chrono::Utc;
//
//     #[test]
//     fn cache_test() {
//         let context: TestContext = TestContext::new();
//         let mut cache = super::RedisCache::new_with_client(context.client, &Auth0Config::test_config());
//         cache.set("ciao", CacheEntry::new("strunz".to_string(), Utc::now(), Utc::now())).unwrap();
//         let result = cache.get("ciao").unwrap();
//         assert!(result.is_some());
//         assert_eq!(result.unwrap().token, "strunz".to_string());
//         let result = cache.get("beo").unwrap();
